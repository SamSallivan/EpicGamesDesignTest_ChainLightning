...
public class CameraBob : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (weapon.isActiveAndEnabled && weapon.GetChargeTime() > 0.1f)
		{
            float targetFOV = defaultFOV + 6f;

            if (weapon.weaponControllerList[weapon.currentWeaponIndex].attackIndex == 3) {
				targetFOV = defaultFOV - 25f;
            }

            //GetComponent<Camera>().fieldOfView = Mathf.LerpUnclamped(GetComponent<Camera>().fieldOfView, targetFOV, fovCurve.Evaluate(weapon.GetChargeTime()) / 1.5f);
            GetComponent<Camera>().fieldOfView = Mathf.Lerp(GetComponent<Camera>().fieldOfView, targetFOV, Time.deltaTime * 15f);
        }
		else
		{
			GetComponent<Camera>().fieldOfView = Mathf.Lerp(GetComponent<Camera>().fieldOfView, defaultFOV + (PlayerController.instance.rb.isKinematic ? 15f : 0f), Time.deltaTime * 15f);
        }

        //GetComponent<Camera>().fieldOfView = Mathf.Lerp(GetComponent<Camera>().fieldOfView, defaultFOV, Time.deltaTime);
